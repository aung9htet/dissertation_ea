...
def ea_singlecore(n, benchmark_func, repeat, max_runtime = 1000, cnf_file = 0):
    ...
    if benchmark_func == 2:
        ...
        for i in prepare_data:
            run_times, optimum, best_fitness = ea(i)
...
def get_data(max_bit, repeat, max_runtime = None, benchmark_func = 0, multicore = True, cnf_file = 0):
    ...
    if benchmark_func == 2:
        ...
        if cnf_file == 0:
            ...
            text = 'uf250'
        if multicore == True:
            run_times, optimum_total, best_fitness_total = ea_multiprocessing(n, benchmark_func, repeat, max_runtime = max_runtime, core = 10, cnf_file = cnf_file)
        else:
            run_times, optimum_total, best_fitness_total = ea_singlecore(n, benchmark_func, repeat, max_runtime = max_runtime, cnf_file = cnf_file)
        # save run times
        ...
        optimum_file = "results/ea_" + text + "_optimum_list.npy"
        data = np.asarray(optimum_total)
        np.save(optimum_file, data)
        # save best_fitness per run time list
        best_fitness_file = "results/ea_" + text + "_best_fitness_list.npy"
        ...
        for n in range(5, max_bit+1):
            if multicore == True:
                run_time = ea_multiprocessing(n, benchmark_func, repeat, core = 6)
            else:
        ...
        np.save(file, data)
    sys.stdout.write('Results has been saved!')
    sys.stdout.flush()
